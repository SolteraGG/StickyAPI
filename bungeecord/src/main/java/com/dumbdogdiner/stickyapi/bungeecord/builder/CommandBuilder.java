...
public class CommandBuilder extends CommandBuilderBase<CommandBuilder> {
    ...
    public interface TabExecutor {
        public java.util.List<String> apply(
            CommandSender sender,
            String commandLabel,
            Arguments args
        );
    ...
    private void performExecution(
        ...
        if (args.size() > 0 && getSubCommands().containsKey(args.get(0))) {
            CommandBuilder subCommand = (CommandBuilder) getSubCommands()
                .get(args.get(0));
            if (!getSynchronous() && subCommand.getSynchronous()) {
                throw new RuntimeException(
                    "Attempted to asynchronously execute a synchronous sub-command!"
                );
            }
            ...
            if (getSynchronous() && !subCommand.getSynchronous()) {
                subCommand.performAsynchronousExecution(
                    sender,
                    builder,
                    label,
                    argsClone
        ...
        variables.put(
            ...
            cooldownSenders.containsKey(sender)
                ? String.valueOf(
                    getCooldown() -
                    (System.currentTimeMillis() - cooldownSenders.get(sender))
                )
                : "0"
        );
        try {
    ...
    private static class TabableCommand
        ...
        public TabableCommand(CommandBuilder builder) {
            super(
                ...
                builder.getAliases().toArray(new String[0])
            );
        ...
        public Iterable<String> onTabComplete(
            ...
            if (builder.tabExecutor != null) return builder.tabExecutor.apply(
                sender,
                builder.getName(),
                new Arguments(Arrays.asList(args))
            ); else {
                if (args.length == 0) {
                ...
                ProxiedPlayer senderPlayer = sender instanceof ProxiedPlayer
                    ? (ProxiedPlayer) sender
                    : null;

                ArrayList<String> matchedPlayers = new ArrayList<String>();
                for (ProxiedPlayer player : ProxyServer
                    .getInstance()
                    .getPlayers()) {
