...
public class Translation {
    ...
    private static String pluralize(String lvalue, String arg) {
        ...
        if (lvalue == "1") return singular; else return plural;
    }

    private static String yesno(String lvalue, String arg) {
        if (arg.isEmpty()) return Boolean.valueOf(lvalue)
            ? "yes"
            : "no"; else if (arg.contains(",")) return arg.split(
            ","
        )[Boolean.valueOf(lvalue) ? 0 : 1]; else return Boolean.valueOf(lvalue)
            ? arg
            : "no";
    }
    ...
    public static TreeMap<String, BiFunction<String, String, String>> functions = new TreeMap<String, BiFunction<String, String, String>>(
...

        {
            put(
                "pluralize",
                (String lvalue, String arg) -> {
                    return pluralize(lvalue, arg);
                }
            );
            put(
                "datetime",
                (String lvalue, String args) -> {
                    return lvalue == null || lvalue == ""
                        ? "Never"
                        : (new SimpleDateFormat(args)).format(
                                Timestamp.valueOf(lvalue)
                            );
                }
            );
            ...
            put(
                "duration",
                (String lvalue, String unused) -> {
                    return lvalue == null || lvalue == ""
                        ? "Never"
                        : TimeUtil.durationString(Long.valueOf(lvalue));
                }
            );
            put(
                "expiry",
                (String lvalue, String unused) -> {
                    return lvalue == null || lvalue == ""
                        ? "Never"
                        : TimeUtil.expirationTime(Long.valueOf(lvalue));
                }
            );
            put(
                "cut",
    ...
    public static String translateColors(String chars, String message) {
        ...
        if (chars == null) return message;

        // Don't allocate if we don't have to.
        if (!message.contains(chars)) return message;

        StringBuilder retstr = new StringBuilder(message);
        for (
            int pos = message.indexOf(chars);
            pos != -1;
            pos = message.indexOf(chars, pos)
        ) {
            if (pos + 1 > message.length()) break;
            ...
            if (Translation.isxdigit(message.charAt(pos + 1))) {
                ...
                pos += 2;
            } else pos += 2; // Skip 2 characters, invalid sequence.
        }
     ...
     * @return {@link java.lang.String}
     */
    public static String translateVariables(
        LocaleProvider locale,
        String message,
        Map<String, String> Variables
    ) {
        // If it doesn't have the starting char for variables, skip it.
        if (!message.contains("{") || Variables == null) return message;

        String retstr = message;
        // Try and iterate over all our variables.
        for (
            int pos = retstr.indexOf("{"), pos2 = retstr.indexOf("}", pos);
            pos != -1 && pos2 != -1;
            pos = retstr.indexOf("{", pos + 1), pos2 =
                retstr.indexOf("}", pos + 1)
        ) {
            // If we're longer than we should be.
            if (pos + 1 > retstr.length() || pos2 + 1 > retstr.length()) break;
            ...
            if (variable.contains("|")) {
                ...
                if (value == null) value = locale.get(lvalue);
                ...
                if (rvalue.contains(":")) {
                    ...
                    replacement =
                        functions.get(rvalue.trim()).apply(value, argument);
                } else replacement = // Variables.containsKey(lvalue.trim())) // (Functions.containsKey(rvalue.trim()) &&
                    functions.get(rvalue.trim()).apply(value, "");
            } else if (
                Variables.containsKey(variable) || locale.get(variable) != null
            ) {
                // Now we replace it with our value from the map.
                if (Variables.containsKey(variable)) replacement =
                    Variables.get(variable); else replacement =
                    locale.get(variable);
            }

            if (replacement != null) retstr =
                retstr.substring(0, pos) +
                replacement +
                retstr.substring(pos2 + 1);
        }
        return retstr;
    }
    ...
    public static String translate(
        ...
        String retstr = Translation.translateVariables(
            locale,
            message,
            Variables
        );
