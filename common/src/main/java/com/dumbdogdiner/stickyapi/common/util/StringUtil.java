...
public final class StringUtil {
    ...
    public static String createProgressBar(
        ...
        for (double i = 0; i < size; i++) {
            if (i < barCount) {
                if (!monospace) barBuilder.append("\u258D"); // ...
        ...
        if (includePercentage) barBuilder.append(
            String.format(" %s%%", percentageFormatter.format(percentage))
        );
     ...
     * @return {@link String}
     */
    public static String capitaliseSentence(
        @NotNull String string,
        @NotNull Boolean keepCase
    ) {
     ...
     * @return {@link String}
     */
    public static String capitaliseSentenceKeepUpperCase(
        @NotNull String string
    ) {
    ...
    public static String replaceLeet(@NotNull String message) {
        ...
        for (Map.Entry<String, String> entry : leetReplace.entrySet()) message =
     ...
     * @return {@link Boolean}
     */
    public static boolean compareMany(
        @NotNull String haystack,
        @NotNull String[] needles
    ) {
        for (String needle : needles) {
            if (haystack.equalsIgnoreCase(needle)) return true;
        }
    ...
    public static UUID hyphenateUUID(@NotNull String uuid) {
        if (uuid.length() == 32) {
            return UUID.fromString(
                uuid.replaceFirst( // https://stackoverflow.com/a/19399768
