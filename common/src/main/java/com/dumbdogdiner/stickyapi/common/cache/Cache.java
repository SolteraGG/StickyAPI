...
public class Cache<T extends Cacheable> {
    ...
    @Getter
    private FutureTask<?> objectExpiryTask = new FutureTask<>(
        new Callable<Boolean>() {
            ...
            public Boolean call() {
                ...
                objectInsertionTimestamps.forEach(
                    (k, v) -> {
    ...
    public T get(@NotNull String key) {
        ...
        if (object != null) debug.print(
            "Got cached entry for " + clazz.getSimpleName() + " with key " + key
        );
    ...
    public T find(@NotNull CacheTester<T> tester) {
        ...
        debug.print(
            "Failed to find " + clazz.getSimpleName() + " using parsed matcher"
        );
        return null;
    }
    ...
    public void put(@NotNull T object) {
        ...
        objectInsertionTimestamps.put(
            object.getKey(),
            System.currentTimeMillis()
        );
        debug.print(
            "Created cached entry for " +
            clazz.getSimpleName() +
            " with key " +
            object.getKey()
        );
