...
final class Luhn {
    ...
    public static boolean luhnCheck(String card) {
        if (card == null || card.length() == 0) return false;
    ...
    public static String calculateCheckDigit(String card) {
        ...
        for (int i = 0; i < card.length(); i++) digits[i] =
            Character.getNumericValue(card.charAt(i));
...
public class ShortID {
     ...
     * @return {@link ShortID}
     */
    public static ShortID fromString(String name)
        throws IllegalArgumentException {
        if (!validateID(name)) throw new IllegalArgumentException(
            "The provided String is not a valid Luhn-parseable ShortID"
        );
    ...
    public static ShortID generate() {
        return generateBase(
            Integer.parseInt(
                UUID.randomUUID().toString().replace("[A-z\\-]", "")
            )
        );
    }
    ...
    public static boolean validateID(@NotNull String shortId) {
        Preconditions.checkNotNull(shortId, "shortId cannot be null");
