...
public class Arguments {
    ...
    @Getter
    private final ArrayList<String> unparsedArgs;
    ...
    private Arguments optionalStringImplementation(
        String name,
        String fallback
    ) {
        ...
        if (unparsedArgs.size() > position) {
            ...
            debug.print(
                "Could not find string, using default value of " + fallback
            );
     ...
     * @return {@link com.dumbdogdiner.stickyapi.common.arguments.Arguments}
     */
    public Arguments optionalString(
        @NotNull String name,
    ...
    public Arguments requiredString(String name) {
        ...
        if (unparsedArgs.size() > position) {
            parsedArgs.put(name, unparsedArgs.get(position));
            debug.print("Found string at position " + position);
            position++;
        } else {
    ...
    private Arguments optionalSentenceImplementation(
        String name,
        String fallback,
        int length
    ) {
        ...
        debug.print(
            "Looking for optional sentence - start = " +
            String.valueOf(position) +
            ", end = " +
            String.valueOf(end) +
            ", length = " +
        ...
        if (position >= end) {
            debug.print(
                "Start cannot be greater than or equal to end, using default value of " +
                fallback
            );
            parsedArgs.put(name, fallback);
            return this;
        ...
        if (unparsedArgs.size() < position + length) {
            debug.print(
                "Could not find sentence of appropriate length (args are size " +
                String.valueOf(position) +
    ...
    public Arguments optionalSentence(@NotNull String name) {
        debug.print(
            ...
            String.valueOf(unparsedArgs.size() - position)
        );
     ...
     * @return {@link com.dumbdogdiner.stickyapi.common.arguments.Arguments}
     */
    public Arguments optionalSentence(
        @NotNull String name,
    ...
    public Arguments optionalSentence(
        ...
        if (fallback != null) {
            ...
            debug.print(
                "Explicit fallback string of null attempted for parameter " +
                name +
                ", argument not added."
            );
            return this;
     ...
     * @return {@link com.dumbdogdiner.stickyapi.common.arguments.Arguments}
     */
    public Arguments optionalSentence(@NotNull String name, int length) {
    ...
    public Arguments requiredSentence(@NotNull String name) {
        debug.print(
            "Using default length: " + (unparsedArgs.size() - position)
        );
     ...
     * @return {@link com.dumbdogdiner.stickyapi.common.arguments.Arguments}
     */
    public Arguments requiredSentence(@NotNull String name, int length) {
        int end = position + length;
        debug.print(
            "Looking for required sentence - start = " +
            position +
            ", end = " +
            end +
            ", length = " +
        ...
        debug.print("Found sentence of length " + length);
    ...
    public Arguments optionalTimeString(@NotNull String name) {
        ...
        if (
            unparsedArgs.size() > position &&
            TimeUtil.toTimestamp(unparsedArgs.get(position)) != null
        ) {
            parsedArgs.put(
                name,
                String.valueOf(
                    TimeUtil.toTimestamp(unparsedArgs.get(position)).getTime()
                )
            );
            unparsedArgs.remove(position);
            debug.print(
                "Found timestamp at position " +
                String.valueOf(position) +
                " - new args size = " +
                String.valueOf(unparsedArgs.size())
            );
        } else debug.print("Could not find timestamp");
    ...
    private Arguments optionalIntImplementation(
        @NotNull String name,
        ...
        if (
            unparsedArgs.size() > position &&
            NumberUtil.isNumeric(unparsedArgs.get(position))
        ) {
            parsedArgs.put(name, unparsedArgs.get(position));
            position++;
            debug.print(
                "Found int at position " +
                position +
                " - new args size = " +
                unparsedArgs.size()
            );
            ...
            debug.print(
                "Could not find int, using default value of " + fallback
            );
    ...
    public Arguments optionalInt(
        ...
        if (fallback != null) {
            ...
            debug.print(
                "Explicit fallback integer of null attempted for parameter " +
                name +
                ", argument not added."
            );
            return this;
    ...
    public Arguments optionalDuration(@NotNull String name) {
        ...
        if (
            unparsedArgs.size() > position &&
            TimeUtil.duration(unparsedArgs.get(position)).isPresent()
        ) {
            parsedArgs.put(name, unparsedArgs.get(position));
            unparsedArgs.remove(position);
            debug.print(
                "Found duration at position " +
                position +
                " - new args size = " +
                unparsedArgs.size()
            );
        } else debug.print("Could not find duration");
    ...
    public Long getDuration(@NotNull String name) {
        return TimeUtil.duration(parsedArgs.get(name)).isPresent()
            ? TimeUtil.duration(parsedArgs.get(name)).get()
            : null;
    }
}
