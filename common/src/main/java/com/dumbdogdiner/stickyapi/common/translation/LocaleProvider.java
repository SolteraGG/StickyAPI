...
public class LocaleProvider {
    ...
    public boolean loadLocale(@NotNull File file) {
        debug
            .reset()
            .print("Looking for localization in " + file.getName() + "...");

        if (!file.exists() || file.isDirectory()) {
            debug.print(
                "Could not find file - does not exist, or is directory"
            );
            return false;
        }
        ...
        if (!locale.getIsValid()) {
            debug.print(
                "Encountered an error while loading the locale configuration - skipping load"
            );
            return false;
        }
    ...
    public int loadAllLocales() {
        ...
        for (File file : this.localeFolder.listFiles()) {
            ...
            if (loadLocale(file)) ++accumulator;
        }
     ...
     * @return {@link java.lang.String}
     */
    public String translateNoColor(
        @NotNull String node,
        @NotNull Map<String, String> vars
    ) {
     ...
     * @return {@link java.lang.String}
     */
    public String get(@NotNull String name, @NotNull String node)
     ...
     */
    public Locale getLocale(@NotNull String name)
        throws IllegalArgumentException {
        checkForLoadedLocale(name);
     ...
     */
    private void checkForLoadedLocale(@NotNull String name)
        throws IllegalArgumentException {
        if (
            !loadedLocales.containsKey(name)
        ) throw new IllegalArgumentException(
            "Locale " + name + " is not loaded"
        );
    }
