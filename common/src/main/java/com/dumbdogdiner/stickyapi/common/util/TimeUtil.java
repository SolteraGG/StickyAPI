...
public final class TimeUtil {
    ...
    public static String significantDurationString(@NotNull final long time) {
        ...
        if (message.length() == 0) {
            message.append(
                new DecimalFormat("0.00").format(timeSince /= 30.0) + "/mo"
            );
        }
    ...
    public static String durationString(@NotNull final long time) {
        ...
        if (years != 0) {
            components.add(
                String.format("%d %s", years, years != 1 ? "years" : "year")
            );
        }
        if (weeks != 0) {
            components.add(
                String.format("%d %s", weeks, weeks != 1 ? "weeks" : "week")
            );
        }
        if (days != 0) {
            components.add(
                String.format("%d %s", days, days != 1 ? "days" : "day")
            );
        }
        if (hours != 0) {
            components.add(
                String.format("%d %s", hours, hours != 1 ? "hours" : "hour")
            );
        }
        if (minutes != 0) {
            components.add(
     ...
     * @return {@link java.lang.String}
     */
    public static String significantDurationString(
    ...
    private static String expTime(
        @NotNull long time,
        @NotNull boolean compact
    ) {
        long currentTime = System.currentTimeMillis();
        if (time == 0) return "never expires"; else if (
            time < currentTime
        ) return String.format(
            "%s ago",
            compact
                ? TimeUtil.significantDurationString(time)
                : TimeUtil.durationString(time)
     ...
     * @since 2.0 - This can no longer take <code>null</code> as a value
     */
    public static String significantExpirationTime(
        @NotNull Timestamp timestamp
    ) {
        return significantExpirationTime(
    ...
    public static Optional<Long> duration(@NotNull String string) {
        ...
        for (var i = 0; i < string.length(); ++i) {
            ...
            if ((ch >= '0') && (ch <= '9')) subtotal =
                (subtotal * 10) + (ch - '0'); else {
                ...
                if (multiplier == null) return Optional.empty();
    ...
    public static Timestamp toTimestamp(@NotNull String timePeriod) {
        boolean compare = StringUtil.compareMany(
            timePeriod,
            new String[] { "*", "0" }
        );
    ...
    private static Timestamp _toTimestamp(String timePeriod) {
        ...
        if (dur.isPresent()) return (
                ((TimeUtil.getUnixTime() + dur.get()) * 1000L) >=
                (253402261199L * 1000L)
            )
            ? null
