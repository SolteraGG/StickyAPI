...
public class ReflectionUtilInvokeProtectedMethodClassTest {
    ...
    public void testInvokeProtectedMethodClass() {
        try (
            MockedStatic<ReflectionUtilInvokeProtectedMethodClassTest> mocked = mockStatic(
                ReflectionUtilInvokeProtectedMethodClassTest.class
            )
        ) {
            mocked
                .when(
                    ReflectionUtilInvokeProtectedMethodClassTest::privateStaticMethod
                )
                .thenCallRealMethod();

            ReflectionUtil.invokeProtectedMethod(
                (Class<?>) ReflectionUtilInvokeProtectedMethodClassTest.class,
                "privateStaticMethod"
            );
    ...
    public void testInvokeProtectedMethodClassArgsString() {
        try (
            ...
            mocked
                .when(
                    () ->
                        ReflectionUtilInvokeProtectedMethodClassTest.privateStaticMethodArgsString(
                            anyString()
                        )
                )
                .thenCallRealMethod();
    ...
    public void testInvokeProtectedMethodClassArgsInteger() {
        try (
            ...
            mocked
                .when(
                    () ->
                        ReflectionUtilInvokeProtectedMethodClassTest.privateStaticMethodArgsInt(
                            anyInt()
                        )
                )
                .thenCallRealMethod();
    ...
    public void testInvokeProtectedMethodObject() {
        ...
        assertTrue(
            (boolean) ReflectionUtil.getProtectedValue(c, "privateMethodHasRun")
        ); // set to true while running the method
    }
}
