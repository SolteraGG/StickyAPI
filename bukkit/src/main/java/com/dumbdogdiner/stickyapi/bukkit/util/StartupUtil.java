...
public class StartupUtil {
    ...
    public static boolean setupConfig(@NotNull JavaPlugin plugin) {
        try {
            if (!plugin.getDataFolder().exists()) {
                plugin
                    .getLogger()
                    .info("Error: No folder was found! Creating...");
                if (!plugin.getDataFolder().mkdirs()) {
                    plugin
                        .getLogger()
                        .info(
                            "Error: Unable to create data folder, are your file permissions correct?"
                        );
                    return false;
                }
     ...
     * @return False if something went wrong
     */
    public static LocaleProvider setupLocale(
        @NotNull JavaPlugin plugin,
        @Nullable LocaleProvider localeProvider
    ) {
        localeProvider =
            new LocaleProvider(new File(plugin.getDataFolder(), "locale"));
        int loadedLocales = localeProvider.loadAllLocales();
        boolean localeEnabled = localeProvider.setDefaultLocale(
            "messages.en_us"
        );
        ...
        if (!localeEnabled) {
            plugin
                .getLogger()
                .severe(
                    "Failed to configure default locale file - perhaps you deleted it? Will create a new one."
                );
            // FIXME: This is horrible and needs to be improved
            try {
                localeProvider.writeLocaleStream(
                    plugin.getResource("messages.en_us.yml"),
                    "messages.en_us.yml",
                    true
                );
            } catch (Exception e) {
                ...
                plugin
                    .getLogger()
                    .severe(
                        "Something went horribly wrong while saving the default locale."
                    );
                return null;
            }
            ...
            .getLogger()
            .info("Loaded " + String.valueOf(loadedLocales) + " localizations");
