...
public class CommandBuilder extends CommandBuilderBase<CommandBuilder> {
        ...
        try {
            if (
                ...
                if (
                    ...
                    this.getRequiresPlayer() && !(sender instanceof Player)
                ) {
    ...
    public org.bukkit.command.Command build(@NotNull Plugin plugin) {
        ...
        if (this.getAliases() != null) command.setAliases(this.getAliases());
    ...
    public void register(@NotNull Plugin plugin) {
        ...
        if (ServerVersion.isPaper()) {
            plugin
                .getServer()
                .getCommandMap()
                .register(plugin.getName(), this.build(plugin));
            return;
        }
        // However, if it's not running paper, we need to use reflection, which is
        // really annoying
        (
            (CommandMap) ReflectionUtil.getProtectedValue(
                plugin.getServer(),
                "commandMap"
            )
        ).register(plugin.getName(), this.build(plugin));
